...
async function handleLogin(event, session) {
  ...
  const localCount = localPlaylists.length + localLayouts.length;
  const cloudCount = (cloud?.playlists?.length || 0) + (cloud?.layouts?.length || 0);

  // Cloud wins when it's newer — but never let an empty cloud wipe real local data
  // (guards against a browser-with-empty-storage accidentally uploading zeros).
  const cloudWins = cloudTs > localTs && (cloudCount > 0 || localCount === 0);
  ...
  if (cloudWins) {
    ...
    scheduleSave(); // local is newer (or cloud is empty) → push up
  }
