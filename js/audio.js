...
// On iOS, ctx.resume() is async; calling ctx.currentTime before it
// resolves gives a stale value, causing all notes to burst together.
function whenRunning(fn) {
  if (!ctx) { initAudio(); fn(); return; }
...
export function playNote(noteName) {
  initAudio();
  whenRunning(() => playFreq(noteToFreq(noteName)));
}
...
export function playScale(noteNames) {
  ...
  return all.map((note, i) => ({ note, delayMs: Math.round(i * INTERVAL * 1000) }));
}
